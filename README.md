...
> 이 프로젝트는 개인의 공간 경험(qualitative observation)을 공공데이터와 수제 데이터셋(quantitative evidence)으로 교차 검증하는 디지털인문학(Digital Humanities) 실험이기도 하다.
...
├── assets/                         # 전시 스크린샷
├── data/
│   └── libraries_2019_2025.csv     # 가공한 도서관 데이터셋 (220개 관)
└── scripts/
    ├── make_comparison_map.py      # Map 01 · 2019↔2025 구내식당 비교 지도 생성
    └── make_facility_map.py        # Map 02 · 운영 주체별 식당·카페 분포 지도 생성
```

전시 사이트에 임베드된 전자지도(`pinakes-me.github.io/seoul-library-cafeteria/` 등)는 위 스크립트로 생성한 결과물이며, 별도 레포지토리에서 배포된다.

---

## 재현 방법 · How to Reproduce

데이터와 코드를 모두 공개해, 누구나 동일한 지도를 다시 생성할 수 있도록 했다.

```bash
# 1. 의존성 설치
pip install folium

# 2. 지도 생성 (data/ 폴더의 CSV를 읽어 HTML 지도를 출력)
python scripts/make_comparison_map.py   # → libraries_2019_2025_comparison.html
python scripts/make_facility_map.py      # → libraries_facility_2025_by_owner.html
```

생성된 HTML 파일을 브라우저에서 열면 인터랙티브 지도를 확인할 수 있다. 데이터셋(`data/libraries_2019_2025.csv`)은 도서관별 위·경도와 연도별 편의시설 보유 여부를 담고 있으며, 편의시설 컬럼을 수정하면 갱신된 데이터로 지도를 다시 그릴 수 있다.
...
<sub>© 사적인북마크 · 2026 · 데이터 출처는 각 기관에 있으며, 수집·해석의 책임은 제작자에게 있습니다.</sub>
